...
void parallel_computer::initialize() {
	...
	for (s32 t = 0; t < num_workers; t++) {
		thread::spawn		(-1, worker_func, "parallel_computer", 0, (void*)this);
	}
...
void parallel_computer::worker_func(void *args) {
	...
	while (true) {
        ...
        if (stop) {
			thread::interlocked_dec(&worker_params->_thread_counter);
			worker_params->_master_sem.signal();
            break;
        }
        // do work
        ...
        // signal requester
        if ((task.num_chunks) == thread::interlocked_inc(task.chunk_counter)) {
			task.completion_sem->signal();
        }
    }
}
...
void parallel_computer::launch_task_and_wait(task_func *func, void *args, u32 work_size) {
	...
	thread::semaphore completion_sem;
...
void parallel_computer::assign_workload(vector<parallel_task> &tasks, u32 work_size, volatile s32 *chunk_counter, void *args, task_func *func, thread::semaphore *sem) {
...
void parallel_computer::post_tasks(vector<parallel_task> &tasks) {
    ...
    for (s32 i = 0; i < num_threads; i++) {
         while (!_task_queue.try_push_task_queue(tasks[i])) {
             puts				(LOG_WARNING "Not enough space in the parallel tasks queue!");
			 thread::yield	();
         }
    }
